...
（原 v2 版主张与"待补的洞"见 §9b。W20–W41 回收后于 2026-09-07 冻结，2026-09-08 为"普适性 + 修目标"解冻：W42 已回收，W43/W44 进行中。）

**2026-09-08 增补（W42，详见 §9.10q）**：(i) **普适结构**：16 个模型里 14 个有一个 BOS/sink token 支配的早期 down_proj（45–833 倍），GPTQ 在那里于模板位置输 RTN >2 倍；例外 gemma-2 两个尺寸（无支配、检测器静默、规则无益）。(ii) **从机制推出的修法**：token 归一化 Hessian（一行代码，c4 不变）治愈 Llama .150→.648/.668、Q14 .412→.720。(iii) **阈值**：Llama 十个臂由"第 2 层输出模板 token 处误差 1.4–1.5"完美分开，横跨比特、group、damping、校准、模块干预；4-bit 在阈值下（1.3），3-bit g32 仍在阈值上（2.4）—— 决定崩塌的是 sink 层模板位置的误差，不是名义比特或全局误差。主语改为 "the layer-wise reconstruction objective"，GPTQ 是载体。
